...
class Unit {
  ...
  Unit({required this.unitId, required this.name, required this.createdAt});
...
class _UnitManagementPageState extends State<UnitManagementPage> {
  ...
  Future<void> fetchUnits({String? search}) async {
    setState(() {
      isLoading = true;
    });
    ...
    try {
      final url = search != null && search.isNotEmpty
          ? Uri.parse(
              'http://192.168.17.133:3001/api/units/units/search/$search',
            )
  ...
  Future<void> updateUnit(Unit unit) async {
    try {
      ...
      final response = await http.put(
        Uri.parse('http://192.168.17.133:3001/api/units/units/${unit.unitId}'),
        headers: {
          'Content-Type': 'application/json',
          'X-User-ID': userId.toString(), // ส่ง user_id ไปกับ header
        },
  ...
  Future<void> deleteUnit(int unitId) async {
    try {
      ...
      final response = await http.delete(
        Uri.parse('http://192.168.17.133:3001/api/units/units/$unitId'),
        headers: {
          'X-User-ID': userId.toString(), // ส่ง user_id ไปกับ header
        },
      );
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        ...
                        children: [
                          Icon(
                            unit == null ? Icons.add : Icons.edit,
                            color: Colors.teal,
                            size: 32,
                          ),
                          SizedBox(width: 10),
                          Text(
                            unit == null ? 'ເພີ່ມຫົວໜ່ວຍ' : 'ແກ້ໄຂຫົວໜ່ວຍ',
                            style: TextStyle(
                              fontSize: 22,
  ...
  void _showDeleteConfirmationDialog(int unitId) async {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Row(
          children: [
            Icon(Icons.rule_sharp, color: Colors.white),
            SizedBox(width: 8),
            Text(
              'ຫົວໜ່ວຍຂອງສິນຄ້າ',
              style: TextStyle(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          ...
                          suffixIcon: IconButton(
                            ...
                            onPressed: () =>
                                fetchUnits(search: searchController.text),
                          ),
                        ),
                        onChanged: (value) => fetchUnits(search: value),
                      ),
                    ),
                  ],
                ),
